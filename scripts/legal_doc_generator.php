...
function main(array $argv): int
{
    try {
        ...
        if ($options['docs'] === 'tos' || $options['docs'] === 'both') {
            ...
            $tosFile = $generator->saveDocument($tosContent, 'TERMS_OF_SERVICE.md', $options['output-dir']);
            $generatedFiles[] = $tosFile;
        }

        if ($options['docs'] === 'privacy' || $options['docs'] === 'both') {
            $privacyContent = $generator->generatePrivacyPolicy();
            $privacyFile = $generator->saveDocument($privacyContent, 'PRIVACY_POLICY.md', $options['output-dir']);
